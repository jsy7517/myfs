...
void set_inode_info_d(superblock * sb);
void makesingle(datablock * db, int blocks, FILE * ifp);
...
void mycpfrom(char * tmp2, char * tmp3)
{
	...
	blocks = size / 128;
	remain = size % 128;
	tmp = find_free_datablock(sb -> freedatablock[0]);
	if(size <= 128){
...
void makesingle(datablock * db, int blocks, FILE * ifp)
{
	...
	int count1 = 0;
		...
		tmp2 = find_free_datablock(sb -> freedatablock[0]);	
		for(int i = 0; i < blocks; i++){
			if(count1 % 5 == 4){
				count1 + 1;
				i = i - 1;
			}
				get_single(db[tmp + 1].data, count1, single[i]);
			if(sing){
				set_datablock_info(&(in[tmp - blocks].datablock_info[1]), tmp2);
				sing = false;
				}
			count1++;
		}
...
{
    static int i;
    int tmp = 0, size, blocks = 0, remain = 0;
	bool check = true;
    tmp = find_free_datablock(sb -> freedatablock[i]);
    db = db + tmp;
    while(1){

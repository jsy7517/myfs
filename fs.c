...
void myrmdir(char * a){
    tree * pre;
    tree * con;
    int inode_num = 0, datablock_num = 0;
...
void myrm(char * a){
    tree * pre;
    tree * con;
    int inode_num = 0, datablock_num = 0, last_db_num = 0;
	...
	datablock_num = in[inode_num].datablock_info[0];
	last_db_num = in[inode_num].datablock_info[2];
    freesuperblock_i(inode_num, sb);
	freesuperblock_d(datablock_num, sb);
...
void myfs_shell(){
    ...
    while(1){
        ...
        if(my){
			...
			else if(!strncmp(tmp1, "myrm", 4) && tmp[4] == '\0')
				myrm(tmp2);

...
void makedouble(char * filename, int blocks, FILE * ifp);
void makesingle(char * filename, int, FILE *);
void makedirect(char * filename, FILE *);
...
void mycpfrom(char * tmp2, char * tmp3)
{
	...
	if(size <= 128){
		...
		makesingle(tmp2, blocks, ifp);
	}
	else{
		makedouble(tmp2, blocks, ifp);
	}
}
...
void makedirect(char * filename, FILE * ifp) //direct 생성
{
	int a, b;
...
void makesingle(char * filename, int blocks, FILE * ifp) // single indirect 생성
{
